-- ============================================================
-- ترقية: منع المستثمر من الاستثمار في مشروعه الخاص
-- تُحدّث دالة الدخول لتُرجع رقم مشروع المستثمر أيضاً (يحتاجه
-- التطبيق للمقارنة مع رقم مشروع كل شركة قبل السماح بالتقييم)
-- شغّل هذا كاملاً في SQL Editor بمشروع Supabase
-- ============================================================

-- التوقيع القديم كان يُرجع عمودين فقط (id, name)؛ نحذفه أولاً
-- لأن تغيير الأعمدة المُرجَعة يتطلب DROP قبل CREATE OR REPLACE
...
-- تحقق: يجب أن تظهر 3 أعمدة بالنتيجة (id, name, project_number)
select routine_name, data_type from information_schema.routines
where routine_name = 'investor_login_or_register';
